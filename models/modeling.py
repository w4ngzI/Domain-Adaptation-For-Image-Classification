...
class Transformer(nn.Module):
    ...
    def forward(self, input_ids, ad_net, is_source=False):
        ...
        #embedding_output = [32, 257, 768]  posi_emb=[1, 257, 768]
...
class Decoder(nn.Module):
    ...
    def forward(self, x):
        ...
        out = out.view(-1, 3, 16, 16)    #[8192, 3, 16, 16]
...
class VisionTransformer(nn.Module):
    ...
    def __init__(self, config, img_size=224, num_classes=21843, zero_head=False, vis=False, msa_layer=12):
        ...
        # with torch.no_grad():
            # nn.init.zeros_(self.tmp.weight)
            # nn.init.zeros_(self.tmp.bias)
        self.decoder = Decoder(config.hidden_size)
        self.relu = nn.LeakyReLU()
        self.BN = nn.BatchNorm1d(config.hidden_size // 2)
        ##########纵向####################################
        self.multilayer_mmd_1 = Linear(config.hidden_size, config.hidden_size//2)
        self.multilayer_mmd_2 = Linear(config.hidden_size//2, num_classes)
        
        # #################################################
        
        ########################################################################
        # self.multilayer_mmd_1 = Linear(config.hidden_size, config.hidden_size)
        # self.multilayer_mmd_2 = Linear(config.hidden_size, config.hidden_size // 2)
        # self.multilayer_mmd_3 = Linear(config.hidden_size//2, config.hidden_size//4)
        # self.multilayer_mmd_4 = Linear(config.hidden_size//4, num_classes)
        ########################################################################
    ...
    #     #x_s=[32, 257, 768]
    #     logits_s = self.head(x_s[:, 0])     #[32, 65]
        
    #     if x_t is not None:
    #         xt_unfold = F.unfold(x_t, kernel_size=16, stride=16)    #[32, 768, 256]
    #         x_t, loss_ad_t, _, _ = self.transformer(x_t, ad_net)    #x_t=[32, 257, 768]
    #         logits_t = self.head(x_t[:, 0]) #[32, 65]

    #         rec_t = self.decoder(x_t[:, 1:])    #[32, 256, 3, 16, 16]
    ...
    def forward(self, x_s, x_t=None, ad_net=None):
        ...
        logits_s = self.head(x_s[:, 0])     #[32, 65]
        ################################################################
        # cls_logits_s = self.head(x_s[:, 0])
        # logits_s_1 = self.multilayer_mmd_1(x_s[:, 0])
        # logits_s_1 = self.relu(logits_s_1)
        # logits_s_1 = self.BN(logits_s_1)
        # logits_s = self.multilayer_mmd_2(logits_s_1)
        ################################################################
        
        ################################################################
        # logits_s_1 = self.multilayer_mmd_1(x_s[:, 0])
        # logits_s_1 = self.relu(logits_s_1)
        # logits_s_2 = self.multilayer_mmd_2(logits_s_1)
        # logits_s_2 = self.relu(logits_s_2)
        # logits_s_3 = self.multilayer_mmd_3(logits_s_2)
        # logits_s_3 = self.relu(logits_s_3)
        # logits_s = self.multilayer_mmd_4(logits_s_3)
        ################################################################
        ...
        if x_t is not None:
            ...
            logits_t = self.head(x_t[:, 0]) #[32, 65]
            #############################################################
            # cls_logits_t = self.head(x_t[:,0])
            # logits_t_1 = self.multilayer_mmd_1(x_t[:, 0])
            # logits_t_1 = self.relu(logits_t_1)
            # logits_t_1 = self.BN(logits_t_1)
            # logits_t = self.multilayer_mmd_2(logits_t_1)
            #############################################################
            
            ################################################################
            # logits_t_1 = self.multilayer_mmd_1(x_t[:, 0])
            # logits_t_1 = self.relu(logits_t_1)
            # logits_t_2 = self.multilayer_mmd_2(logits_t_1)
            # logits_t_2 = self.relu(logits_t_2)
            # logits_t_3 = self.multilayer_mmd_3(logits_t_2)
            # logits_t_3 = self.relu(logits_t_3)
            # logits_t = self.multilayer_mmd_4(logits_t_3)
            ################################################################
            ...
            loss_rec = self.criterion(rec_t, xt_unfold)
            return logits_s, logits_t, (loss_ad_s + loss_ad_t) / 2.0, loss_rec, x_s, x_t
            # return logits_s, logits_t, (loss_ad_s + loss_ad_t) / 2.0, loss_rec, \
            #     [x_s, logits_s_1],\
            #     [x_t, logits_t_1]
            # return logits_s, logits_t, (loss_ad_s + loss_ad_t) / 2.0, loss_rec, \
            #     [x_s, logits_s_1, logits_s_2, logits_s_3],\
            #     [x_t, logits_t_1, logits_t_2, logits_t_3]
        else:
            return logits_s, attn_s, tran_s
    ################################################################
    ...
    def load_from(self, weights):
        with torch.no_grad():
            ...
            # if self.zero_head:
                # print(self.zero_head)  #True
            # nn.init.zeros_(self.multilayer_mmd_1.weight)
            # nn.init.zeros_(self.multilayer_mmd_1.bias)
            # nn.init.zeros_(self.multilayer_mmd_2.weight)
            # nn.init.zeros_(self.multilayer_mmd_2.bias)
            # else:
            #     self.tmp.weight.copy_(np2th(weights["head/kernel"]).t())
            #     self.tmp.bias.copy_(np2th(weights["head/bias"]).t())
