...
def train(args, model):
    ...
    for global_step in tqdm(range(1, t_total)):
        ...
        # print('logits_s', logits_s.shape)    #[32, 65]
        # print('logits_t', logits_t.shape)    #[32, 65]
        ...
        ########for multi layer method################################################################################
        # loss_ad_global = lossZoo.adv(torch.cat((feature_s[0][:,0], feature_t[0][:,0]), 0), ad_net)
        ################################################################################################################################################
        
        ########original method######################################################################
        # target = nn.Softmax(dim=1)(logits_t)
        # loss_bnm = BNM(target)
        ...
        # loss_bnm = BNM(target)
        loss_func = MMDLoss()
        loss_MMD = loss_func(feature_s[:, 0], feature_t[:, 0])
        # loss_awl = awl(loss_MMD, loss_ad_global)
        # # print(awl.params)
        # awl_loss_list.append(awl.params.detach().numpy().copy())
        # loss = loss_clc + args.beta * loss_awl + args.gamma * loss_ad_local #+ loss_bnm

        loss = loss_clc + args.beta * (loss_MMD+loss_ad_global) + args.gamma * loss_ad_local
        if args.use_im:
            loss += (args.theta * loss_im)
        #################################################################################################
        
        #only single layer mmd loss
        #################################################################################################
        # loss_func = MMDLoss()
        # loss_MMD = loss_func(feature_s[:, 0], feature_t[:, 0])
        # loss = loss_clc + args.beta * loss_MMD + args.gamma * loss_ad_local
        ...
        ###############################################################################################
        # loss_func = MMDLoss()
        # loss_1 = loss_func(feature_s[0][:, 0], feature_t[0][:, 0])
        # loss_2 = loss_func(feature_s[1], feature_t[1])
        # loss_MMD = awl(loss_1, loss_2)
        # awl_loss_list.append(awl.params.detach().numpy().copy())
        # # loss_MMD = loss_1 + loss_2
        # loss = loss_clc + args.beta * loss_MMD + args.gamma * loss_ad_local
        # if args.use_im:
        #     loss += (args.theta * loss_im)
        #####################################################################################################\
        ...
        # loss_MMD = loss_1 + loss_2
        # # # loss_clc_1 = loss_fct(feature_s[2].view(-1, args.num_classes), y_s.view(-1))
        # # # loss_cls_combined = awl_cls(loss_clc, loss_clc_1)
        # loss_3 = loss_func(feature_s[2], feature_t[2])
        # loss_4 = loss_func(feature_s[3], feature_t[3])
        # loss_MMD = awl(loss_1, loss_2, loss_3, loss_4)
        # loss = loss_clc + args.beta * loss_MMD + args.gamma * loss_ad_local
        # if args.use_im:
        #     loss += (args.theta * loss_im)
        #####################################################################################################
        ...
        #multi adversarial loss
        #########################################################################################
        # loss_ad_global_1 = lossZoo.adv(torch.cat((feature_s[0][:,0], feature_t[0][:,0]), 0), ad_net)
        # loss_ad_global_2 = lossZoo.adv(torch.cat((feature_s[1], feature_t[1]), 0), ad_net_1)
        # print(loss_ad_global_1, loss_ad_global_2)
        # loss_ad_global = awl(loss_ad_global_1, loss_ad_global_2)
        
        # loss_ad_global = loss_ad_global_1 + 0.1 * loss_ad_global_2
        # loss = loss_clc + args.beta * loss_ad_global + args.gamma * loss_ad_local
        ...
        if global_step % args.eval_every == 0 and args.local_rank in [-1, 0]:
            ...
            if best_acc < accuracy:
                ...
                best_acc = accuracy
                # torch.save(model, "clip_RW.pth")
                # torch.save(ad_net_local, "clip_ad_net.pth")
    ...
    np.save("loss_awl_2layer_mmd.npy", awl_loss_list)
